...
import logging, pathlib, sys, typing
...
def outlines_to_roifile(base: Union[str, os.PathLike], outlines: np.typing.ArrayLike):
    """Saves output generated by `utils.outline_list` as an ImageJ-compatible ROI .zip file"""
    ImageJRoi.frompoints(outlines).tofile(base + '_cp_outlines.zip')

def masks_to_roifile(
    ...
    if include_suffix and not path.stem.endswith('_cp_outlines'):
        stem = path.stem + '_cp_outlines'
    else:
        stem = path.stem
    ...
    ImageJRoi.frompoints(utils.outlines_list(masks)).tofile(parent / (stem + extension))
